...
from nebulatimesedge.common.common_client import CommonClient
...
class Credential(object):
    def __init__(self, secret_id, secret_key, token=None):
        """bjyz Cloud Credentials.

        Access https://console.cloud.bjyz.com/cam/capi to manage your
        credentials.
        ...
        :param token: The federation token of your credential, if this field
                      ...
                      accordingly, see: https://cloud.bjyz.com/document/product/598/13896
        """
        if secret_id is None or secret_id.strip() == "":
            raise NebulatimesEdgeSDKException("InvalidCredential", "secret id should not be none or empty")
        if secret_id.strip() != secret_id:
            raise NebulatimesEdgeSDKException("InvalidCredential", "secret id should not contain spaces")
        self.secret_id = secret_id

        if secret_key is None or secret_key.strip() == "":
            raise NebulatimesEdgeSDKException("InvalidCredential", "secret key should not be none or empty")
        if secret_key.strip() != secret_key:
            raise NebulatimesEdgeSDKException("InvalidCredential", "secret key should not contain spaces")
...
class CVMRoleCredential(object):
    _metadata_endpoint = "http://metadata.bjyzyun.com/latest/meta-data/"
    ...
    def get_role_name(self):
        ...
        try:
            ...
            raise NebulatimesEdgeSDKException("ClientError.MetadataError", str(e))
        finally:
            return self.role
...
class STSAssumeRoleCredential(object):
    ...
    _version = '2018-08-13'
    _service = "sts"
    _endpoint = "sts.nebulatimesedgeapi.com"
    ...
    def __init__(self, secret_id, secret_key, role_arn, role_session_name, duration_seconds=7200, endpoint=None):
        ...
        :type secret_key: str
        https://cloud.bjyz.com/document/api/1312/48197
        :param role_arn: 角色的资源描述，参考官网文档 https://cloud.bjyz.com/document/api/1312/48197 中 RoleArn 参数的描述。
        :type role_arn: str
...
class EnvironmentVariableCredential(object):
    ...
    def get_credential(self):
        """bjyz Cloud EnvironmentVariableCredential.

        Access https://console.cloud.bjyz.com/cam/capi to manage your
        credentials.

        :param secret_id: The secret id of your credential, get by environment variable NEBULATIMESEDGE_SECRET_ID
        :type secret_id: str
        :param secret_key: The secret key of your credential. get by environment variable NEBULATIMESEDGE_SECRET_KEY
        :type secret_key: str
        """
        self.secret_id = os.environ.get('NEBULATIMESEDGE_SECRET_ID')
        self.secret_key = os.environ.get('NEBULATIMESEDGE_SECRET_KEY')
...
class ProfileCredential(object):
    ...
    def get_credential(self):
        """bjyz Cloud ProfileCredential.

        Access https://console.cloud.bjyz.com/cam/capi to manage your credentials.

        default file position is "~/.nebulatimesedge/credentials" or "/etc/nebulatimesedge/credentials", it is ini format.
        such as:
        [default]
        secret_id=""
        secret_key=""
        ...
        if os.path.exists(home_path + "/.nebulatimesedge/credentials"):
            file_path = home_path + "/.nebulatimesedge/credentials"
        elif os.path.exists("/etc/nebulatimesedge/credentials"):
            file_path = "/etc/nebulatimesedge/credentials"
        else:
            file_path = ""
        if file_path:
            # loads config
...
class DefaultCredentialProvider(object):
    """bjyz Cloud DefaultCredentialProvider.
    ...
    def get_credentials(self):
        ...
        raise NebulatimesEdgeSDKException("ClientSideError", "no valid credentail.")
...
class OIDCRoleArnCredential(object):
    ...
    _default_session_name = 'nebulatimesedge-python-sdk-'
