...
class ClientProfile(object):
    ...
    def __init__(self, signMethod=None, httpProfile=None, language="zh-CN",
        ...
        if language not in valid_language:
            raise NebulatimesEdgeSDKException("ClientError", "Language invalid, choices: %s" % valid_language)
...
class RegionBreakerProfile(object):
    ...
    def __init__(self, backup_endpoint="ap-guangzhou.nebulatimesedgeapi.com",
                 max_fail_num=5,
        ...
        :param backup_endpoint: Backup region of endpoint, default ap-guangzhou.nebulatimesedgeapi.com.
        ...
        if not self.check_endpoint():
            raise NebulatimesEdgeSDKException("ClientError", "the format of `backup_endpoint` must be nebulatimesedgeapi.com "
                                                          "or ${region}.nebulatimesedgeapi.com")
        ...
        if self.max_fail_percent < 0 or self.max_fail_percent > 1:
            raise NebulatimesEdgeSDKException("ClientError", "max fail percent must be set between 0 and 1")
    ...
    def check_endpoint(self):
        ...
        if len(endpoint_split) != 3 and len(endpoint_split) != 2:
            return False
        if endpoint_split[-2] != "nebulatimesedgeapi" or endpoint_split[-1] != "com":
            return False
