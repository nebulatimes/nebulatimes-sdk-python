...
import nebulatimesedge
from nebulatimesedge.common.exception.nebulatimes_edge_sdk_exception import NebulatimesEdgeSDKException
from nebulatimesedge.common.exception import NebulatimesEdgeSDKException as SDKError
from nebulatimesedge.common.http.request import ApiRequest, ResponsePrettyFormatter
from nebulatimesedge.common.http.request import RequestInternal
from nebulatimesedge.common.profile.client_profile import ClientProfile, RegionBreakerProfile
from nebulatimesedge.common.sign import Sign
from nebulatimesedge.common.circuit_breaker import CircuitBreaker
from nebulatimesedge.common.retry import NoopRetryer

warnings.filterwarnings("ignore", module="nebulatimesedge", category=UserWarning)
...
LOGGER_NAME = "nebulatimesedge_sdk_common"
...
class AbstractClient(object):
    ...
    _endpoint = ''
    _service = ''
    _sdkVersion = 'SDK_PYTHON_%s' % nebulatimesedge.__version__
    ...
    def _format_params(self, prefix, params):
        ...
        raise NebulatimesEdgeSDKException("ClientParamsError", "some params type error")
    ...
    def _build_req_inter(self, action, params, req_inter, options=None):
        ...
        if options.get('SkipSign'):
            ...
            raise NebulatimesEdgeSDKException("ClientError", "Invalid signature method.")
    ...
    def _check_status(self, resp_inter):
        if resp_inter.status_code != 200:
            ...
            raise NebulatimesEdgeSDKException("ServerNetworkError", resp_inter.content)
    ...
    def _check_error(self, resp):
        ...
        if "Error" in data["Response"]:
            ...
            raise NebulatimesEdgeSDKException(code, message, reqid)
    ...
    def _call(self, action, params, options=None, headers=None):
        ...
        if not isinstance(headers, dict):
            raise NebulatimesEdgeSDKException("ClientError", "headers must be a dict.")
    ...
    def _call_with_region_breaker(self, action, params, options=None, headers=None):
        ...
        try:
            ...
            return resp
        except NebulatimesEdgeSDKException as e:
